...
int main(int argc, const char* argv[]) {
    ...
    ssd_get_output(ssd_output, scores, bboxes, anchors, strides, 0.5, 0.4);

    printf("detected %d objs\n", ssd_output.size());
    for (const auto& out : ssd_output)
    {
        printf("cls: %d  prob: %f  bbox: %f %f %f %f\n", out.cls, out.prob, out.bbox.fLeft, out.bbox.fTop, out.bbox.fRight, out.bbox.fBottom);
        cv::putText(img, VOC_NAMES[out.cls], cv::Point(out.bbox.x() * w_scale, out.bbox.y()* h_scale), cv::FONT_HERSHEY_COMPLEX, 1, cv::Scalar(0,0,255),1);
